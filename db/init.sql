...
DROP FUNCTION IF EXISTS add_quote(VARCHAR(255), VARCHAR(255)); 
CREATE FUNCTION add_quote(quote VARCHAR(255), user_name VARCHAR(255))
RETURNS VOID AS
$$
DECLARE
    id_count int;
    user_id int;
BEGIN
    SELECT count(users.user_id) INTO id_count
    FROM users
...
---------------------------------------------
-- TODO: Don't inveture function signature --
---------------------------------------------

-- DROP FUNCTION IF EXISTS add_comment(VARCHAR(255), VARCHAR(255)); -- name collisions $1 - 
-- CREATE FUNCTION add_comment(comment VARCHAR(255), user_name VARCHAR(255))
-- RETURNS VOID AS 
-- $$
-- DECLARE
--     id_count int;
...
SELECT add_quote('There are only two kinds of languages: the ones people complain about and the ones nobody uses.', 'Bjarne Stroustrup');
